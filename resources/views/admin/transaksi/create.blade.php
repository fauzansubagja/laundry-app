    ...
    <div class="row">
        <div class="col-lg-6">
            ...
            <div class="mb-3 col-md-12">
                ...
                <select class="multi-select" name="paket_id[]" multiple="multiple">
...
<script>
    ...
    function getPaketPrice(ids) {
        $.ajax({
            url: '/transaksi/get-price',
            method: 'POST',
            data: JSON.stringify({
                "ids": ids
            }),
            ...
            success: function(response) {
                document.getElementById("total_biaya").value = response.total_harga
            },
            error: function(xhr) {
    ...
    $('#paket_id').on('change', function() {
        ...
        if (ids && ids.length > 0) {
            ...
            // mengambil total biaya dari inputan dan menambahkan harga setiap paket yang dipilih
            var total_biaya = 0;
            for (var i = 0; i < ids.length; i++) {
                var harga = $('#' + ids[i]).data('harga');
                total_biaya += harga;
            }
    ...
    // fungsi untuk mengubah nilai diskon menjadi persentase
    function addPercent() {
    ...
    $('#diskon').on('change', function() {
        ...
        if (diskon && total_biaya) {
            ...
            var biaya_setelah_diskon = total_biaya - diskon_biaya;
            $('#total_biaya').val(biaya_setelah_diskon);
        }
    });
</script>
